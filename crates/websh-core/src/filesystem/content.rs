...
mod tests {
    ...
    impl StorageBackend for StubBackend {
        ...
        fn scan(
            &self,
        ) -> crate::storage::BoxFuture<
            '_,
            crate::storage::StorageResult<crate::storage::ScannedSubtree>,
        > {
            Box::pin(async { Ok(crate::storage::ScannedSubtree::default()) })
        }

        fn read_text<'a>(
            &'a self,
            rel_path: &'a str,
        ) -> crate::storage::BoxFuture<'a, crate::storage::StorageResult<String>> {
        ...
        fn read_bytes<'a>(
            &'a self,
            rel_path: &'a str,
        ) -> crate::storage::BoxFuture<'a, crate::storage::StorageResult<Vec<u8>>> {
            self.reads.lock().unwrap().push(rel_path.to_string());
            let text = self.text.clone();
        ...
        fn commit<'a>(
            &'a self,
            _request: &'a crate::storage::CommitRequest,
        ) -> crate::storage::BoxFuture<
            'a,
            crate::storage::StorageResult<crate::storage::CommitOutcome>,
        > {
            Box::pin(async {
                Err(crate::storage::StorageError::BadRequest(
