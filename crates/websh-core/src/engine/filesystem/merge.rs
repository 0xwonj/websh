...
use crate::domain::{ChangeSet, ChangeType};
...
pub fn apply_all_changes_to_global(fs: &mut GlobalFs, changes: &ChangeSet) {
    let deleted_dirs = delete_directory_paths(changes, false, &VirtualPath::root());
    for (path, entry) in changes.iter_all() {
        if is_descendant_of_deleted_directory(path, &deleted_dirs) {
            continue;
        }
...
pub fn apply_staged_changes_to_global_for_root(
    ...
    let deleted_dirs = delete_directory_paths(changes, true, root);
...
fn delete_directory_paths(
    changes: &ChangeSet,
    staged_only: bool,
    root: &VirtualPath,
) -> Vec<VirtualPath> {
    let iter = if staged_only {
        changes.iter_staged().collect::<Vec<_>>()
    } else {
        changes.iter_all().collect::<Vec<_>>()
    };
    iter.into_iter()
        .filter(|(path, entry)| {
            path.starts_with(root) && matches!(entry.change, ChangeType::DeleteDirectory)
        })
        .map(|(path, _)| path.clone())
        .collect()
}

fn is_descendant_of_deleted_directory(path: &VirtualPath, deleted_dirs: &[VirtualPath]) -> bool {
    deleted_dirs
        .iter()
...
mod tests {
    ...
    fn delete_removes_from_merged() {
        ...
        let mut cs = ChangeSet::new();
        upsert(&mut cs, p("/a.md"), ChangeType::DeleteFile);
    ...
    fn delete_directory_removes_subtree_and_pending_content() {
        ...
        let mut cs = ChangeSet::new();
        upsert(&mut cs, p("/a"), ChangeType::DeleteDirectory);
        let merged = merged(&fs, &cs);
    ...
    #[test]
    fn delete_directory_suppresses_descendant_updates_in_view() {
        let mut fs = base();
        fs.upsert_directory(p("/docs"), dir_meta("docs"));
        fs.upsert_file(
            p("/docs/old.md"),
            "old".into(),
            file_meta(),
        ...
        upsert(&mut cs, p("/docs"), ChangeType::DeleteDirectory);

        let merged = merged(&fs, &cs);

        assert!(merged.get_entry(&p("/docs")).is_none());
        assert!(merged.read_pending_text(&p("/docs/old.md")).is_none());
    }

    #[test]
    fn delete_directory_suppresses_descendant_creates_in_view() {
        let mut fs = base();
        fs.upsert_directory(p("/docs"), dir_meta("docs"));
        ...
        assert!(merged.get_entry(&p("/docs")).is_none());
        assert!(merged.read_pending_text(&p("/docs/new.md")).is_none());
    }
