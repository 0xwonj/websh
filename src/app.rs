...
use crate::core::storage::GitHubBackend;
use crate::core::FsState;
use crate::models::{
...
/// Terminal output uses a [`RingBuffer`] for O(1) push operations.
///
/// # Note
///
...
/// - **Filesystem**: FsState with base VirtualFs, pending changes, and memoized MergedFs
...
impl AppContext {
    ...
    pub fn new() -> Self {
        ...
        use crate::core::storage::local::load_pending_changes;

        let mounts = configured_mounts();

        // Load pending changes from localStorage
        let pending = load_pending_changes().unwrap_or_default();

        Self {
            mounts: StoredValue::new(MountRegistry::from_mounts(mounts)),
            fs: FsState::new(pending),
    ...
    /// Get a GitHub backend instance for the first writable mount.
    pub fn get_github_backend(&self) -> Option<GitHubBackend> {
        self.mounts.with_value(|registry| {
            registry
                .all()
                .find(|m| m.is_writable())
                .and_then(|m| GitHubBackend::from_storage(&m.storage, m.content_prefix.clone()))
        })
    }
