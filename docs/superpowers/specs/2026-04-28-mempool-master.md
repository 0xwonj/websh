...
| A9 | **Reserved URL prefixes** for the hash-router: `/`, `/ledger`, `/websh`, `/explorer`, `/new`. Content repo (`/site` via `BOOTSTRAP_SITE`) and mempool repo MUST NOT introduce files or directories whose top-level URL segment collides with this list. | Added 2026-04-29 (Phase 6); narrowed 2026-04-29 (Phase 7) by dropping `/edit/`. `/new` is claimed by the URL-driven mempool authoring flow (the page mounts in Edit mode with a frontmatter placeholder); `/ledger`/`/websh`/`/explorer` are claimed by the existing router; `/` is the home route. Phase 7 abandoned the dedicated `/edit/<path>` URL: view/edit toggle now happens in-page with no URL change, so `/edit/` is no longer reserved. A future content file at `content/new.md` would still be silently shadowed and unreachable. |
...
| 6 | Reader-Unified Mempool UI | Drop modals; mempool participates in URL navigation. `/<path>` view, `/edit/<path>` edit, `/new` compose. `Reader` and both modal components deleted; `MempoolEditor` + `MempoolEditorPage` host the un-modal'd compose form | **Superseded by Phase 7** |
| 7 | Single-Component View/Edit Reader | One `RendererPage` with view/edit toggle. URL unchanged across the toggle; raw-markdown editing including frontmatter; `/new` opens the same component in Edit with placeholder. `MempoolEditor`, `MempoolEditorPage`, and the `/edit/<path>` route deleted. | **Complete** |
| 8 | Reader Component Refinement | Phase 7's edit/save/cancel buttons moved out of `SiteChrome.extra_actions` (site-scoped) into a Reader-internal toolbar (document-scoped). `RendererPage` renamed to `Reader`. Toolbar matches the ledger.html aesthetic (dashed rule, monospace, accent buttons). Adds a `preview` button (Edit→View round-trip) and `draft_dirty` flag preserves user content across the round-trip. `SiteChrome.extra_actions` prop dropped (no callers). | **Complete** |
| 9 | Mempool Collapse/Expand | Click the mempool header to fold the entries list. Chevron `▾`/`▸` indicator, keyboard (Enter/Space), `aria-expanded` + `aria-controls`. Session-only state lifted to `LedgerPage` so filter-route changes don't reset it. | **Complete** |

After Phase 9, V1 is complete (Phases 4–9 are post-Phase-3 hardening + architectural pivots + UX consolidations + design refinement + mempool UX polish — see Decision Log). V2 items (§7) are queued separately.
...
| 6 | Design | `docs/superpowers/specs/2026-04-29-mempool-phase6-design.md` | Approved (superseded) |
| 6 | Plan | `docs/superpowers/plans/2026-04-29-mempool-phase6-plan.md` | Complete (superseded) |
| 7 | Design | `docs/superpowers/specs/2026-04-29-mempool-phase7-design.md` | Approved |
| 7 | Plan | `docs/superpowers/plans/2026-04-29-mempool-phase7-plan.md` | Complete |
| 8 | Design | `docs/superpowers/specs/2026-04-29-mempool-phase8-design.md` | Approved |
| 8 | Plan | `docs/superpowers/plans/2026-04-29-mempool-phase8-plan.md` | Complete |
| 9 | Design + Plan | `docs/superpowers/specs/2026-04-29-mempool-phase9-design.md` | Complete |
...
| 2026-04-29 | Phase 7 — Phase 6's structured compose form (`MempoolEditor`) and dedicated edit URL (`/#/edit/<path>`) replaced with a single-component view/edit toggle on `RendererPage`. URL unchanged across the toggle (HackMD-style); the textarea edits raw markdown including frontmatter, no separate metadata fields. `/#/new` opens the same component in Edit with a frontmatter placeholder. New helpers: `save_raw` (raw-bytes commit), `derive_new_path` (parses frontmatter to compute target path), `placeholder_frontmatter`. Post-save invalidation via `refetch_epoch` + `LocalResource::refetch()` belt-and-suspenders. A9 reserved-prefix list narrowed to drop `/edit/`; `/new` stays reserved. `MempoolEditor`, `MempoolEditorPage`, `edit_request_path_inner` deleted (~860 lines). 5 implementation commits across steps 1–5. Reviewer findings closed in this batch: design pass 1 (3 CRITICAL + 5 IMPORTANT + several MINOR), design pass 2 (1 CRITICAL refetch + 2 IMPORTANT clarifications), plan pass (2 CRITICAL + 4 IMPORTANT + 4 suggestions). | §3 A9, §4 |
| 2026-04-29 | Phase 9 — Mempool section gains a collapse/expand toggle. Click anywhere on the `.mpHead` (or focus + Enter/Space) folds the entries list; the count stays visible. Chevron `▾` (expanded) / `▸` (collapsed) at the left of the label gives the visual indicator. The compose link's `on:click=stop_propagation()` keeps clicking compose from accidentally folding the section. ARIA: `role="button"`, `tabindex="0"`, `aria-expanded`, `aria-controls="mempool-rows"`. Collapse state is a session-only `RwSignal<bool>` lifted to `LedgerPage` (not `Mempool`) because `LedgerPage`'s `move || ledger.get().map(...)` re-runs on filter-route changes — a `Mempool`-local signal would reset on every `/#/ledger` ↔ `/#/writing` flip. Persistence across hash navigation is intentionally out of scope (CLAUDE.md "no localStorage in feature code"); follow-up work would route the preference through `AppContext`. Reviewer findings closed: 1 CRITICAL (filter remount drops state — fixed by lifting), 2 IMPORTANT (aria-expanded, stop_propagation is load-bearing), 4 suggestions (stated in design v2). | §3 |
| 2026-04-29 | Phase 8 — Reader component design refinement. Phase 7 wired edit/save/cancel into `SiteChrome.extra_actions`, conflating document-level actions with site-level chrome. Phase 8 corrects the placement: `RendererPage` renamed to `Reader` (`src/components/reader.rs`); a private `ReaderToolbar` sub-component lives at the top of the article frame with `[edit]` (View) or `[preview] [cancel] [save]` (Edit), styled to match the ledger.html aesthetic (dashed border-bottom, monospace, accent buttons). New `preview` button flips Edit→View without saving (HackMD-style toggle); a `draft_dirty: RwSignal<bool>` flag prevents `on_toggle_edit` from re-seeding from `raw_source` after a preview round-trip. `SiteChrome.extra_actions` prop dropped (no callers). 3 implementation commits across steps 1–3. Reviewer findings closed: design pass (3 IMPORTANT — draft preservation, preview/path note, `extra_actions` removal), plan pass (1 HIGH ordering + 2 MEDIUM stylance/draft_body — addressed by reordering steps so Reader extraction lands before the `extra_actions` drop). | §3, §4 |
