...
| 3 | Design | `docs/superpowers/specs/2026-04-30-reader-redesign-phase3-design.md` | Approved |
| 3 | Plan | `docs/superpowers/plans/2026-04-30-reader-redesign-phase3-plan.md` | Complete |
...
| 2026-04-30 | Phase 3 complete. New `reader/toolbar.rs` (~165 lines incl. 6 unit tests) holds the footnote-mark `ReaderToolbar` and `state_label` / `state_class` pure helpers. Inline `ReaderToolbar` removed from `mod.rs`. `ReaderMode` promoted to `pub(super)`. New `dirty` prop wired from `draft_dirty.read_only()`. Window-level `keydown` listener installed via `install_reader_keybindings(KeybindingTargets)` with `on_cleanup` removing it on Reader unmount; the `WasmCleanup` wrapper pattern (unsafe `Send + Sync`, `js_function(&self)` accessor) bridges the wasm-bindgen `Closure` (`!Send`) past Leptos's `Send + Sync + 'static` cleanup bound — sound on wasm32 (single-threaded). Reviewer flagged one HIGH (`Cmd+R` / `Cmd+E` would have hijacked browser shortcuts; fixed by gating the letter match on `!meta_key && !ctrl_key && !alt_key && !in_textarea`) and one MEDIUM (Phase 2's `draft_dirty=true` on Edit entry was invisible under the old toolbar but Phase 3 surfaces it as `● unsaved` before any keystrokes; fixed by splitting into `draft_owned` for the re-seed guard and `draft_dirty` for save-state). Old CSS classes `.toolbar / .toolbarLabel / .toolbarActions / .actionButton / .actionButtonPrimary` removed; new `.modefn*` set added. 546 tests pass; cargo + wasm32 + trunk all green. | §6, §3 |
